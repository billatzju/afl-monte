...
void afl_fsrv_init(afl_forkserver_t *fsrv) {
  ...
  fsrv->state_heap_size = 65536; // Assuming all states start in the heap initially
  FILE *fb = fopen("score_records","a");
  fprintf(fb, "initializing the heap, mapsize %d\n", fsrv->state_heap_size);
  for (u32 i = 0; i < fsrv->state_heap_size; ++i) {

      // 1. Initialize the state_info entry itself
      fsrv->state_info[i].state_id = i;       // Set the correct ID
      fsrv->state_info[i].total_score = 0.0;  // Initialize score (as double)
      fsrv->state_info[i].hit_count = 0;
      fsrv->state_info[i].heap_size = 0;      // Initialize seed heap size
      fsrv->state_info[i].top_seeds_full = 0;
      // Optionally memset top_seeds if needed:
      // memset(fsrv->state_info[i].top_seeds, 0, sizeof(seed_entry_t) * MAX_TOP_SEEDS);

      // // 2. *** CRITICAL: Populate the hash map ***
      // // Add the mapping: state_id 'i' is initially at heap index 'i'.
      if (!state_map_upsert(fsrv, i, i)) {
        
          // Handle error - upsert should succeed unless alloc fails
          PFATAL("Failed to insert initial state %u into state map!", i);
      }
  }
  u32 heap_idx;
  u32 state_ind = 10;
  if(state_map_lookup(fsrv, state_ind, &heap_idx)){
    fprintf(fb, "heap ind for state 10 is %d\n", heap_idx);
  }
  fclose(fb);
}
