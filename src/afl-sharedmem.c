...
u8 *afl_shm_init(sharedmem_t *shm, size_t map_size,
  ...
  shm->translog_map_size = sizeof(trans_log_t);
  shm->trans_log_map = NULL;
#ifdef USEMMAP
  ...
  shm->translog_shm_id = shmget(IPC_PRIVATE, sizeof(trans_log_t), IPC_CREAT | IPC_EXCL | DEFAULT_PERMISSION);
  
  if (shm->translog_shm_id < 0) {
    shmctl(shm->translog_shm_id, IPC_RMID, NULL); // Clean up main map
    PFATAL("shmget() failed for translog");
  }
  if (!non_instrumented_mode) {
    shm_str = alloc_printf("%d", shm->translog_shm_id);
    setenv(SHM_TRANS_LOG, shm_str, 1);
    ck_free(shm_str);
  }
  ...
  shm->trans_log_map =shmat(shm->translog_shm_id, NULL, 0);
