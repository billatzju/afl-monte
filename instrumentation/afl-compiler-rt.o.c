...
void __afl_log_transition(uint32_t prev_location, uint32_t cur_location) {
  u32 state_index = (prev_location << 1) ^ cur_location;
  u32 edge_count = __atomic_fetch_add(&trans_log->count, 1, __ATOMIC_RELAXED);
  if (edge_count >= TRANS_BUF_SIZE) {
    __atomic_fetch_sub(&trans_log->count, 1, __ATOMIC_RELAXED);
    return;
  }

  trans_log->indexes[edge_count] = state_index;
  // FILE *sb = fopen("logging_transitions","a");
  // fprintf(sb, "logging_transition count: %d\n", edge_count);
  // fclose(sb);
}
...
static void __afl_map_shm(void) {
  ...
  char *mapping_log = getenv(SHM_TRANS_LOG);
  // FILE *sb = fopen("sbsbsb","a");
  // fprintf(sb, "translog shared memory region: %s\n", mapping_log);
  // fclose(sb);
  ...
  char *trans_id = getenv(SHM_TRANS_LOG);
  if (trans_id) {
      // FILE *sb = fopen("sbsbsb","a");
      // fprintf(sb, "transition:%s\n", trans_id);
      // fclose(sb);
      int shmid = atoi(trans_id);
      trans_log = shmat(shmid, NULL, 0);
      if (trans_log == (void*)-1) {
          perror("shmat trans_log");
          _exit(1);
      }
  }
