...
struct afl_pass : afl_base_pass {
  ...
  virtual unsigned int execute(function *fn) {
    ...
    FOR_EACH_BB_FN(bb, fn) {
      ...
      if (out_of_line) {
        ...
        tree prev_val = fold_convert(uint32_type_node, ploc); 
        tree cur_val = build_int_cst(uint32_type_node, bid >> 1);
        gcall *trans_call = gimple_build_call(get_afl_log_transition_decl(),
                                            2, prev_val, cur_val);
        gimple_seq_add_stmt(&seq, trans_call);
      }
  ...
  static inline tree get_afl_log_transition_decl() {
    tree type = build_function_type_list(void_type_node,
                                        uint32_type_node, uint32_type_node,
                                        NULL_TREE);
    tree decl = build_fn_decl("__afl_log_transition", type);
    TREE_PUBLIC(decl) = 1;
    DECL_EXTERNAL(decl) = 1;
    return decl;
  }
